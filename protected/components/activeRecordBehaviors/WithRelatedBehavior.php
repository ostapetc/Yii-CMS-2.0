            ...
            foreach ($newData as $name=> $data)
            {
                ...
                if ($relationClass === CActiveRecord::BELONGS_TO)
                {
                    ...
                    foreach ($fks as $i=> $fk)
                    {
                        ...
                        {
                            throw new CDbException(Yii::t('yiiext', 'The relation "{relation}" in active record class "{class}" is specified with an invalid foreign key "{key}". There is no such column in the table "{table}".', array(
                                '{class}'   => get_class($owner),
                                '{relation}'=> $relations[$name],
                                '{key}'     => $fk,
                                '{table}'   => $ownerTableSchema->name
                            )));
                        }

                        if (isset($ownerTableSchema->foreignKeys[$fk]) &&
                            $schema->compareTableNames($relatedTableSchema->rawName, $ownerTableSchema->foreignKeys[$fk][0])
                        )
                        {
                ...
                {
                    $queue[] = array(
                        $relationClass, $relatedClass, $relations[$name]->foreignKey, $name, $data
                    );
                }
            }
            ...
            foreach ($queue as $pack)
            {
                ...
                switch ($relationClass)
                {
                        ...
                        foreach ($fks as $i=> $fk)
                        {
                            if (!isset($relatedTableSchema->columns[$fk]))
                            {
                                throw new CDbException(Yii::t('yiiext', 'The relation "{relation}" in active record class "{class}" is specified with an invalid foreign key "{key}". There is no such column in the table "{table}".', array(
                                    ...
                                    '{relation}'=> $name,
                                    '{key}'     => $fk,
                                    '{table}'   => $relatedTableSchema->name
                                )));
                            }

                            if (isset($relatedTableSchema->foreignKeys[$fk]) &&
                                $schema->compareTableNames($ownerTableSchema->rawName, $relatedTableSchema->foreignKeys[$fk][0])
                            )
                            {
                    ...
                    case CActiveRecord::MANY_MANY:
                        ...
                        {
                            throw new CDbException(Yii::t('yiiext', 'The relation "{relation}" in active record class "{class}" is specified with an invalid foreign key. The format of the foreign key must be "joinTable(fk1,fk2,...)".', array(
                                '{class}'   => get_class($owner),
                                '{relation}'=> $name
                            )));
                        }
                        ...
                        {
                            throw new CDbException(Yii::t('yiiext', 'The relation "{relation}" in active record class "{class}" is not specified correctly: the join table "{joinTable}" given in the foreign key cannot be found in the database.', array(
                                '{class}'    => get_class($owner),
                                '{relation}' => $name,
                                '{joinTable}'=> $matches[1]
                            )));
                        ...
                        foreach ($fks as $i=> $fk)
                        {
                            ...
                            if (isset($joinTable->foreignKeys[$fk]))
                            {
                                ...
                                if (!isset($ownerMap[$pk]) &&
                                    $schema->compareTableNames($ownerTableSchema->rawName, $tableName)
                                )
                                {
                                    $ownerMap[$pk] = $fk;
                                }
                                else if (!isset($relatedMap[$pk]) &&
                                    $schema->compareTableNames($relatedTableSchema->rawName, $tableName)
                                )
                                {
                                    $relatedMap[$pk] = $fk;
                                }
