...
{
    protected $shortDescription, $longDescription, $params = array(), $properties = array(), $authors = array(), $return, $licence, $link, $todo;
    ...
    public function parse($docBlock)
    {
        ...
        foreach ($lines as $line)
        {
            ...
            elseif (preg_match('/@property\s+([^ ]+)?\s+\$([^ ]+)(\s+(.*))?/', $line, $match))
            {
                $this->properties[$match[2]] = array(
                    'type'        => $match[1],
                    'comment' => isset($match[4]) ? $match[4] : '',
                );
            }
